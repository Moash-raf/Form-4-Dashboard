...
df["Shares"] = pd.to_numeric(df["Shares"], errors="coerce").round(2)
...
# --- DATE FILTER SETUP ---
min_date = df["Timestamp"].min().date() if not df["Timestamp"].isna().all() else date.today()
max_date = df["Timestamp"].max().date() if not df["Timestamp"].isna().all() else date.today()
...
def highlight_buy_sell(col):
    if col.name == "Buy/Sell":
        return [
            'background-color: #0A8500; color: white; font-weight:600' if v == "Buy"
            else 'background-color: #B00020; color: white; font-weight:600'
            for v in col
        ]
    return [''] * len(col)
...
st.dataframe(
    filtered_df.style
        .apply(highlight_buy_sell, axis=0)
        .format({"Shares": "{:,.2f}", "Price": "{:,.2f}"}),
    width='stretch',
    height=600
)
